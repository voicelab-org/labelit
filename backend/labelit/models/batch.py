...
class Batch(PolymorphicModel):
    ...
    num_annotators_per_document = models.IntegerField(
        "The target number of annotators per document",
        default=1
    )
    ...
    num_documents = models.IntegerField(
        "The number of documents in this batch (denormalized field)",
        default=1
    )
    ...
    def get_batch_unit(self, document):
        return BatchDocument.objects.get(
            document=document,
            batch=self
        )
    ...
    def get_stats(self):
        ...
        if self.project.is_audio_annotated:
            stats['average_ratio'] = done_annotations.values(
                ...
                ratio=Cast(F('time'), output_field=FloatField()) / Cast(F('document__audio_duration'), output_field=FloatField()),
                time=F('time'),
    ...
    def get_annotation_limit(self):
        if self.annotation_mode == self.EVEN:
            ...
            limit = math.floor(
                self.num_documents * self.num_annotators_per_document / self.annotators.all().count()
            )
            if limit == 0:
                limit = 1
            return limit
    ...
    def get_next_document_to_review(self, user):
        invalidated = Annotation.objects.filter(
            batch=self,
            annotator=user,
            has_qa_invalidated=True,
            is_resubmitted=False,
    ...
    def get_next_document_to_qa(self, skipped_document_ids=[]):
        completed_batch_docs = BatchDocument.objects.filter(
            ...
            document__id__in=skipped_document_ids
        )
        ...
        try:
            return Document.objects.get(
                id=batch_annotations.filter(
                    is_resubmitted=True,
                    has_qa_invalidated=True,
                ).order_by('updated_at').values('document_id').first()['document_id']
            )
        except TypeError:
            try:
                return Document.objects.get(
    ...
    def get_remaining_units_in_dataset(project, dataset):
        return dataset.documents.all().count() - BatchDocument.objects.filter(
    ...
    def get_document_to_undo(self, user):
        doc_id = list(
            self.annotations.filter(
                ...
                num_annotations_in_doc=self.project.tasks.count()
            ).order_by(
                '-last_update'
            )
        )[0]['document_id']
