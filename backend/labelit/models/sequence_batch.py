...
class SequenceBatch(Batch):
    sequences = models.ManyToManyField(
        "labelit.DocumentSequence", through="labelit.BatchDocumentSequence"
    )
    num_sequences = models.IntegerField(
        "The number of document sequences in this batch (denormalized field)", default=1
    )
    ...
    def get_batch_unit(self, document):
        return BatchDocumentSequence.objects.get(
            document_sequence=document.document_sequence, batch=self
        )
    ...
    def get_next_document_to_annotate(self, user):
        ...
        partly_annotated_sequences = (
            Annotation.objects.filter(batch=self, annotator=user, is_done=True)
            .values(
            ...
            .filter(
                num_annotations__lt=num_tasks
                * models.F("document__document_sequence__num_documents")
            )
        )
    ...
    def get_remaining_units_in_dataset(project, dataset):
        return (
            dataset.document_sequences.all().count()
            - BatchDocumentSequence.objects.filter(
                document_sequence__dataset=dataset, batch__in=project.batches.all()
            ).count()
        )
