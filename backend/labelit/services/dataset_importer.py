...
class DatasetImporter:
    ...
    @transaction.atomic
    def _import(self):
        metadata = json.load(open(os.path.join(
        ...
        def _import_documents():
            ...
            document_names = list(set(
                map(lambda x: x.split('.')[0], filenames)
            ))
            for doc_name in document_names:
                doc_dict = dict(dataset=dataset, )
        ...
        dataset_docs = Document.objects.filter(
            dataset=dataset
        )

        def _upload_audio_to_storage():
            audio_doc_names = list(dataset_docs.values_list('audio_filename', flat=True))
            for audio_name in audio_doc_names:
                ...
                audio_storage.open(audio_path, "wb").obj.upload_file(audio_name)
