...
class LabelSerializerTests(TestCase):
    ...
    def setUp(self):
        ...
        self.ord_0 = OrdinalLabel.objects.create(
            task=self.task1,
            index=0
        )
        self.ord_1 = OrdinalLabel.objects.create(
            task=self.task1,
            index=1
        )
        self.ord_2 = OrdinalLabel.objects.create(
            task=self.task1,
            index=2
        )
        self.serializer1 = LabelPolymorphicSerializer(
            instance=self.ord_1
        )
