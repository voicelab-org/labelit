...
from labelit.models import OrdinalTask, Project, \
    SequenceBatch, Document, Dataset, Annotation, \
    DocumentSequence, BatchDocumentSequence, Batch
import math
...
class SequenceBatchModelTests(TestCase):
    ...
    def test_get_annotation_limit_with_annotation_mode_set_to_even(self):
        ...
        self.assertEqual(
            self.batch1.get_annotation_limit(),
            2
        )
    ...
    def test_get_next_document_to_annotate(self):
        self.assertEqual(
            self.batch1.get_next_document_to_annotate(self.user1),
            self.doc2
        )
    ...
    def test_get_remaining_units_in_dataset(self):
        self.assertEqual(
            SequenceBatch.get_remaining_units_in_dataset(self.project, self.dataset),
            0
        )
