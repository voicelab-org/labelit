...
class BatchModelTests(TestSetup, TestCase):
    ...
    def test_get_annotation_limit_with_annotation_mode_set_to_even(self):
        ...
        self.assertEqual(
            batch.get_annotation_limit(),
            20
        )


    def test_get_annotation_limit_with_annotation_mode_set_to_all_you_can_annotate(self):
        batch = Batch.objects.create(
            name="Batch 1",
            dataset=self.dataset,
            project=self.project,
    ...
    def test_get_next_document_to_annotate_in_even_mode(self):
        ...
        self.assertTrue(
            self.batch1.get_next_document_to_annotate(self.user2) in [self.doc1, self.doc2, self.doc3]
        )
    ...
    def test_get_next_document_to_annotate_in_all_you_can_annotate_mode(self):
        ...
        self.assertTrue(
            self.batch1.get_next_document_to_annotate(self.user2) is None
        )

    def test_get_num_done_units(self):
        self.assertEqual(
            self.batch1.get_num_done_units(),
            0
        )
        Annotation.objects.create(
            annotator=self.user1,
    ...
    def test_get_remaining_units_in_dataset(self):
        self.assertEqual(
            Batch.get_remaining_units_in_dataset(self.project, self.dataset),
            0
        )
        Document.objects.create(
            text="a bad movie if there ever there was one",
            dataset=self.dataset
        )
        self.assertEqual(
            Batch.get_remaining_units_in_dataset(self.project, self.dataset),
        ...
        batch2.save()
        self.assertEqual(
            Batch.get_remaining_units_in_dataset(project2, self.dataset),
            3
        )
    ...
    def test_get_document_to_undo(self):
        ...
        self.assertEqual(
            self.batch1.get_document_to_undo(self.user1),
            self.doc1
        )
    ...
    def test_get_next_document_to_review(self):
        # no QA done yet
        self.assertEqual(
            self.batch1.get_next_document_to_review(self.user1),
            None
        )
