...
class ProjectModelTests(TestSetup, TestCase):
    ...
    def test_get_num_done_documents(self):
        self.assertEqual(
            self.project.get_num_done_documents(),
            0
        )
        ...
        for user, task in itertools.product(*[[self.user1, self.user2], [self.task1, self.task2]]):
            Annotation.objects.create(
                annotator=user,
                task=task,
                document=self.doc4,
    ...
    def test_get_num_done_batches(self):
        self.assertEqual(
            self.project.get_num_done_batches(),
            0
        )
