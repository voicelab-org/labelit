...
from labelit.models import Project, Batch, SequenceBatch, \
    Dataset, Document, OrdinalTask
...
class BatchViewSetTests(TestCase):
    ...
    def test_get_next_document_to_annotate(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            'document' in response.data.keys()
        )
        self.assertTrue(
            'annotations' in response.data.keys()
        )
        self.assertEqual(
            len(response.data['annotations']),
            self.project.tasks.all().count()
        )
