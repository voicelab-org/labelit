...
def add_intermediate_blanks_if_needed(inipos, nextstopstart, lcompletestart, lcompleteends, max_len):
...
def force_complete_blank(blk_starts, blk_ends, filelength, max_len):
    ...
    for start, end in zip(blk_starts, blk_ends):
        ...
        add_intermediate_blanks_if_needed(current_time, start, final_blk_starts, final_blk_ends, max_len)
        final_blk_starts.append(start)
        final_blk_ends.append(end)
        current_time = end
    add_intermediate_blanks_if_needed(current_time, filelength, final_blk_starts, final_blk_ends, max_len)

    return final_blk_starts,final_blk_ends

def get_cutpoints(starts_b, ends_b, record_length, min_seg_length, max_seg_length, keep_all_mode):
...
def get_segs(ifile, suppl_args):
    ...
    segs_ends_blank = [analysis_window_length * (x[0] + 2) for x in list(np.argwhere(predictions_diff > 0))]
    if predictions[-1] < 1:
    ...
    if predictions[0] < 1:
        ...
        segs_starts_blank = []
    segs_starts_blank += [analysis_window_length * (x[0] + 1) for x in list(np.argwhere(predictions_diff < 0))]

    # To avoid long segments, we need to insert artificial blanks if some are too far away one from another
    segs_starts_blank, segs_ends_blank = force_complete_blank(segs_starts_blank, segs_ends_blank, filelength, suppl_args['max_seg_length'])

    # Analyze blank area to return "reasonable" cutpoints
    cutpoints = get_cutpoints(segs_starts_blank, segs_ends_blank, filelength, suppl_args['min_seg_length'], suppl_args['max_seg_length'], suppl_args['continuous_transcript'])
    ...
    for one_cutpoint in cutpoints:
        ...
        if suppl_args['music_model'] is not None:
            ...
            if end_point > start_point + 2*sample_rate:
                ...
                to_analyze = signal[start_point + margin: start_point + margin + 2*sample_rate]
            else:
                to_analyze = np.zeros(2*sample_rate)
                to_analyze[0:end_point-start_point] = signal[start_point:end_point]

            spectrogram = librosa.core.stft(y=to_analyze, n_fft=256, win_length=160, hop_length=120)
            spectrogram_power = np.abs(spectrogram) ** 2
            spectrogram_mel = np.log10(np.dot(mel_scale, spectrogram_power) + 1e-6).flatten()
...
def do_trunk(full_filename, dict_args):
    ...
    for pos,one_seg in enumerate(cutpoints):
        ...
        bname = '-'.join([filename, str(pos), '%.2f' % one_seg[0], '%.2f' % one_seg[1]]) + '.wav'
...
def treat_onefile(entryfile):
    dict_args = {'min_seg_length': 1, 'max_seg_length': 20, 'music_model': None, 'continuous_transcript':False}
