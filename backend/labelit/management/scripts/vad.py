...
def safe_flatness(data):
    ...
    output[entirely_not_null_rows] = -10 * np.log10(geometric_mean(data[entirely_not_null_rows, :]) / np.mean(data[entirely_not_null_rows, :], axis=1))
...
class Detector:
    ...
    def __init__(self, signal_sampling_rate=None, window_step=None, stereo_mode=True):
        ...
        self.signal_sampling_rate = signal_sampling_rate if signal_sampling_rate is not None else 8000
        ...
        self.base_energy_threshold = 10 # Multiplier between squared noise power best estimate and "speech"
        ...
        self.post_filter_min_silence_run_points = int(np.round(self.post_filter_min_silence/self.window_step))
        self.post_filter_min_speech_run_points = int(np.round(self.post_filter_min_speech/self.window_step))

        # We work on windows of size...
        self.window_step_points = int(np.round(self.window_step*self.signal_sampling_rate))
        self.window_size_points = int(np.round(2*self.window_step*self.signal_sampling_rate))
        ...
        self.prefilter_energy_threshold = 30 # Multiplier between squared noise power first estimate and "speech"
        if stereo_mode:
    ...
    def __call__(self, audio_data):
        ...
        self.padded_data = np.zeros(self.window_size_points + self.num_windows*self.window_step_points)
        ...
        for window_index in np.where(np.invert(self.low_energy_prefilter))[0]:
            ...
            speech_detected = ((self.logpowers[window_index] >= np.log(self.base_energy_threshold*self.thresh_min_power)) or
                (self.flatness[window_index] >= (self.thresh_min_flatness + self.base_spectral_flatness_threshold)) or
                (self.pitches[window_index] >= (self.thresh_min_pitch + self.base_pitch_threshold)))
            ...
            if speech_detected:
                ...
                self.thresh_min_power = (self.thresh_min_power * self.silence_count + self.powers[window_index]) / (self.silence_count + 1)
    ...
    def extract_stats(self):
        ...
        for window_start in range(0, self.num_windows, self.max_windows):
            ...
            frames = np.array([self.padded_data[win_num*self.window_step_points:win_num*self.window_step_points + self.window_size_points] for win_num in range(window_start, window_end)])
            fft_data = abs(np.fft.fft(frames, self.fft_points)[:,1:self.fft_kept_coeffs])

            # Compute descriptors for current signal portion
            self.pitches[window_start:window_end] = self.fft_bins_freq_increment * np.argmax(fft_data, axis=1)
        ...
        self.low_energy_prefilter = self.logpowers < np.percentile(self.logpowers, self.ini_energy_thresh_percentile) + np.log(self.prefilter_energy_threshold)
        ...
        self.thresh_min_pitch = np.percentile(self.pitches, self.ini_pitch_thresh_percentile)
        self.thresh_min_flatness = np.percentile(self.flatness, self.ini_flatness_thresh_percentile)
    ...
    def post_filter(self):
        ...
        self.ignore_symbol_if_shorter_than(False, self.post_filter_min_silence_run_points)
