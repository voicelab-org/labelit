...
def generate_waveform_from_audio(input_file, output_file, create_folder=False):
    ...
    for idx_chunk in range(num_chunks):
        signal_chunk = signal[
            int(idx_chunk * chunk_size)
            * sr : int((idx_chunk * chunk_size) + chunk_size)
            * sr
        ]
        waveform_intermediate += [
            resampled_val
            for resampled_val in signal_chunk[
                [
                    round(index)
                    for index in np.linspace(0, len(signal_chunk) - 1, points_per_chunk)
                ]
            ]
        ]
    ...
    waveform = [
        float(resampled_val)
        for resampled_val in np_waveform_intermediate[
            [
                round(index)
                for index in np.linspace(
                    0,
                    len(np_waveform_intermediate) - 1,
                    int(settings.NUM_ELEMENTS_IN_WAVEFORM),
                )
            ]
        ]
    ]

    waveform_info = {"duration": duration, "waveform": waveform}
...
def generate_hls_from_audio(
    input_file, output_file, create_folder=False, generate_waveform=True
):
    ...
    cmd = [
        ...
        "segments/segment_%Y%m%d%H%M%S_%%04d_%%08s_%%013t.mp3",
        "-threads",
...
def convert_and_upload_single_file(
    audio_file_name,
    hls_file_name,
    hls_file_key,
    audio_file_key=None,
    ...
    if not check_file_exists(hls_file_key):
        ...
        upload_folder_to_s3_bucket_folder(
            source_path=os.path.dirname(hls_file_name),
...
def check_hls_is_correct(playlist_key):
    ...
    if not check_file_exists(
        os.path.join(os.path.dirname(playlist_uri), "waveform.json")
    ):
        return False
    ...
    with tempfile.TemporaryDirectory() as tmp_dirname:
        ...
        for segment in playlist.segments:
            if not check_file_exists(
                os.path.join(os.path.dirname(playlist_uri), segment.uri)
            ):
                return False
