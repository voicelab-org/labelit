...
class Annotation(models.Model):
    is_done = models.BooleanField("If true, annotation is confirmed", default=False)
    annotator = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
    )
    task = models.ForeignKey("labelit.Task", on_delete=models.CASCADE)  # denormalized
    batch = models.ForeignKey(
        "labelit.Batch", on_delete=models.CASCADE, related_name="annotations"
    )
    ...
    document = models.ForeignKey(
        "labelit.Document", on_delete=models.CASCADE, related_name="annotations"
    )
    ...
    project = models.ForeignKey(
        "labelit.Project", on_delete=models.CASCADE  # denormalized
    )
    ...
    def __str__(self):
        return "<Annotation ({}): task: {} annotator: {}>".format(
            self.pk, self.task, self.annotator
        )
