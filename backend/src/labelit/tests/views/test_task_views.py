...
class TaskViewSetTests(TestCase):
    def setUp(self):
        ...
        self.l1 = Label.objects.create(name="angry", task=self.task)
        self.l2 = Label.objects.create(name="not-angry", task=self.task)
    ...
    def test_get_list_unauthenticated_returns_status_code_401(self):
        ...
        response = TaskViewSet.as_view(actions={"get": "list"})(request)
    ...
    def test_get_a_task_returns_task_with_nested_labels(self):
        request = self.request_factory.get("")
        self._authenticate(request)
        response = TaskViewSet.as_view(actions={"get": "retrieve"})(
    ...
    def test_get_agreement_stats_for_batch(self):
        ...
        request = self.request_factory.get("", {"batch_id": self.batch.pk})
        self._authenticate(request)
        response = TaskViewSet.as_view(
            actions={"get": "get_agreement_stats_for_batch"}
        )(request, pk=self.task.pk)
        self.assertEqual(response.status_code, 200)
        self.assertTrue("metric" in response.data)
