        ...
        def _get_stats(
            ...
            num_annotations_done_by_annotator_on_doc = Subquery(
                Annotation.objects.filter(
                    annotator=OuterRef("annotator"), document=OuterRef("document")
                )
                .values("annotator")
            ...
            if len(projects):
                done_annotations = done_annotations.filter(project_id__in=projects)

            if len(annotators):
                done_annotations = done_annotations.filter(annotator_id__in=annotators)
            ...
            def _add_timing_stats(document_annotator_pairs, stats, by_day=False):
                timing_stats = []
                for pair in document_annotator_pairs:
                    ...
                    if not len(filtered):
                        ...
                        if pair["document__audio_duration"]:
                            filtered[0]["total_duration"] += pair[
                                "document__audio_duration"
                            ]
                            filtered[0]["total_annotation_time"] += pair[
                                "annotation_time"
                            ]
                ...
                for stat in stats:
                    if by_day:
                        ...
                        stat["total_duration"] = list(
                            filter(
                                lambda x: x["annotator"] == stat["annotator"],
                                timing_stats,
                            )
                        )[0]["total_duration"]
                        stat["total_annotation_time"] = list(
            ...
            completed_document_annotator_pairs_per_day = (
                done_annotations.annotate(day=TruncDate(F("updated_at")))
                .values(
                    "document__audio_duration",
                    "document",
                    "annotator",
            ...
            stats_per_annotator_per_day = list(
                ...
                .annotate(num_docs=Count("document", distinct=True))
                .values(
                    "annotator",
                    "annotator__first_name",
                    "annotator__last_name",
