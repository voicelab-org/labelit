          ...
          <v-container>
            ...
            <v-row>
              ...
              >
                ...
                ></v-autocomplete>
                <div v-if="datasetRemainingUnits!=null">Remaining units: {{datasetRemainingUnits}}</div>
              </v-col>
              <v-col cols="12">
                <v-autocomplete
  ...
  export default {
      ...
      return {
        ...
        annotationModeList : [
          {
            ...
            description : "Annotators can annotate as much they want (up to optional limit) By the end of the batch, annotators may have done unequal amounts of work"
          },
          {
            value : "even",
            description : "By the end of the batch, every annotator will have annotated the same number of units"
          }
        ],
        ...
        rules : {
          batchNameRules: [
              ...
              v => (v && v.length <= 60) || 'The batch name must be less than 60 characters',
            ],
          numberOfDocumentsRules :[
              v => !!v || 'The number of documents to include to the batch is required and must be a valid number',
              v => (v && (v >= 1 && v <= this.datasetRemainingUnits ) || 'The number of documents must not exceed the remaining dataset units')
          ],
          numberOfAnnotatorsPerDocumentRules :[
              v => !!v || 'The number of annotators per document is required and must be a valid number',
              v => (v && (v >= 1 && v <= this.datasetRemainingUnits ) || 'The number of annotators per document must not exceed the number of annotators')
          ],
          annotatorsRules : [
              v => !!v.length || 'Select at least one annotator',
          ],
          datasetRules : [
              v => !!v || 'Select a dataset',
          ],
          annotationModeRules : [
              v => !!v || 'Select an annotation mode',
          ],
        }
    ...
    watch: {
        selectedDataset(dataset){
            let vm = this
            ProjectService.getRemainingUnitsInDatasetInProject(vm.projectId, {dataset_id: dataset.id})
            .then(function(response){
