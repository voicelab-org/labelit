<template>
  <div>
    <div class="general-stats" v-if="((numberOfAnnotations && batch) || project) && stats">
      ...
      <table class="stats-table">
        ...
        <template
            v-if="batch">
          <tr>
            <td class="left">Progress (documents fully annotated / total documents)</td>
        ...
        <tr>
          ...
          <td><span v-if="stats.num_done">{{ average_time }} s</span><span v-else>N/A</span></td>
        </tr>
        <tr v-if="proj.is_audio_annotated">
          <td class="left">Average audio time</td>
          <td><span v-if="stats.num_done">{{ toSeconds(stats.average_duration.average) }} s</span><span
              v-else>N/A</span></td>
        </tr>
        <tr v-if="proj.is_audio_annotated">
          <td class="left">Average annotation/audio time ratio</td>
          <td><span v-if="stats.num_done">{{ rounded_ratio }}</span><span v-else>N/A</span></td>
        </tr>
        <tr>
          <td class="left">Number of annotations pending review by annotators</td>
...
export default {
  ...
  methods: {
    ...
    getNumberOfAnnotations() {
      var vm = this
      BatchService.getNumberOfAnnotationsForUser(
          vm.batchId,
      )
          .then(function (res) {
