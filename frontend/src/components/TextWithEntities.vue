...
export default {
  ...
  computed: {
    sortedEntities() {
      return this.annotated_entities.slice().sort((a, b) => a.start_offset - b.start_offset)
    },
  ...
  methods: {
    ...
    updateEntity(startOffset, endOffset, labelId) {
      ...
      // remove old label
      this.annotated_entities = this.annotated_entities.filter(
          (e) => {
    ...
    removeEntity(chunk) {
      ...
      this.annotated_entities = this.annotated_entities.filter(
          (e) => {
            return !(e.start_offset == chunk.start_offset && e.end_offset == chunk.end_offset)
          }
      )
      this.$emit('input', this.annotated_entities)
    },
    ...
    adjustSelection() {
      ...
      var prefix = ""
      if (range.startContainer.nodeName == "#text" && rangeStr[0] != " " && range.startOffset != range.startContainer.textContent.length) {
      ...
      if (range.endContainer.nodeName == "#text" && rangeStr[rangeStr.length - 1] != " " && range.endOffset != 0) {
    ...
    assignLabel(labelId) {
      if (this.annotated_entities.find(e => e.start_offset == this.start && e.end_offset == this.end)) {
