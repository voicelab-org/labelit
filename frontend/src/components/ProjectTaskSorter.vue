  ...
  <div>
    ...
    <div>
      <draggable v-model="sorted_tasks">
        <transition-group>
          <div v-for="(task, index) in sorted_tasks" :key="task.id" class="ordered-task">
            {{ index }} - {{ task.name }}
          </div>
        </transition-group>
      </draggable>
      ...
      handler() {
        ...
        function _edit_sorted_tasks() {
          function _remove_detached_tasks() {
            vm.sorted_tasks = vm.sorted_tasks.filter(
              t => !vm.value.includes(t)
            );
          }

          function _add_new_tasks() {
            let tasks_to_add = vm.value.filter(
              t => !vm.sorted_tasks.map(t => t.id).includes(t.id)
            );
            tasks_to_add.forEach(t => {
    ...
    sorted_tasks: {
      handler() {
        ...
        function _task_sets_match_but_are_ordered_differently() {
          ...
          let sorted_tasks_includes_unique_items =
            vm.sorted_tasks.length !=
            vm.sorted_tasks
              .map(t => t.id)
              .filter(t_id => vm.value.map(t => t.id).includes(t_id)).length;
          let value_includes_unique_items =
            vm.value.length !=
            vm.value
              .map(t => t.id)
              .filter(t_id => vm.sorted_tasks.map(t => t.id).includes(t_id))
              .length;
          let is_order_different = !arrayEquals(sorted_ids, value_ids);

          return (
            !sorted_tasks_includes_unique_items &&
            !value_includes_unique_items &&
            is_order_different
          );
        }
