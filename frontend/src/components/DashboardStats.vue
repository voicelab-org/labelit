  ...
  <div>
    <div v-if="projects && annotators">
      ...
      <div v-if="stats && !loading">
        ...
        <br>
        <b> Total duration of annotated audio: {{ toHours(stats.total_duration) }} hrs </b>
        <br>
        <div class="table-header">
          <h3>Per annotator</h3>
          <ExcelExport :data="stats.stats_per_annotator" :name="'per-annotator-stats'"/>
        </div>
        ...
        <div class="table-header">
          ...
          <ExcelExport :data="stats.stats_per_annotator_per_day" :name="'per-annot-per-day-stats'"/>
        </div>
        <v-data-table
...
export default {
  ...
  methods: {
    ...
    load() {
      ...
      DashboardService.getStats(params).then(
          (res) => {
            this.stats = res.data
