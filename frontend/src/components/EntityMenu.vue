  ...
  >
    ...
    >
      <v-list
          ...
          :ref="'taskList'"
      >
        <span v-shortkey="['enter']" @shortkey="$emit('selected', active_label.id)"></span>
        <span v-shortkey="['arrowup']" @shortkey="browseLabels('up')"></span>
        <span v-shortkey="['arrowdown']" @shortkey="browseLabels('down')"></span>
        <span v-shortkey="['arrowleft']" @shortkey="browseTasks('left')"></span>
        <span v-shortkey="['arrowright']" @shortkey="browseTasks('right')"></span>
        ...
        >
            ...
            <span :class="{active: task == active_task}">
              ...
              >
                  ...
                  >
                    <v-list-item-content>
                      <span v-shortkey="[letter_dict[label.name]]" @shortkey="$emit('selected', label.id)"></span>

                      <span :class="{active: label == active_label}">
                        <v-list-item-title v-text="label.name+ ' ('+letter_dict[label.name]+')'"/>
                      </span>
                    </v-list-item-content>
                  </v-list-item>
              </v-list>
...
export default {
  ...
  computed: {
    ...
    label_list() {
      return this.tasks.map(t => t.labels).reduce((prev, next) => prev.concat(next), [])
    },
    active_label() {
  ...
  methods: {
    ...
    scrollToActive() {
      ...
      this.$nextTick(
          () => {
            this.$refs['task-' + this.active_task.id][0].$el.scrollIntoView({behavior: 'smooth', block: 'nearest', inline: 'start'})
            ...
            console.log("&isVisible(element, container)", isVisible(element, container))
              ...
              this.$nextTick(
                  () => {
                    //if (this.label_list[0] != this.active_label) {
                      this.$refs['label-' + this.active_label.id][0].$el.scrollIntoView({behavior: 'smooth', block: 'nearest', inline: 'start'})
    ...
    browseTasks(direction) {
      if (direction == 'left') {
        ...
        let current_task_idx = this.tasks.findIndex(
            (t) => {
        ...
        if (current_task_idx == 0) {
          ...
          this.cursor_index = this.label_list.findIndex(
              (l) => {
...
span.active > .v-list-item__title, span > .v-list-item__title:hover {
