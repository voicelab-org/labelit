...
let resourcetype_choices = Object.keys(TaskSchemas).map(
    (schema_name) => {
      return {
...
export default {
  ...
  methods: {
    ...
    editLabels(labels, task_type) {
      ...
      labels.forEach(
          l => {
            ...
            let label_keys = Object.keys(l).filter(
                k => k[0] != "_"
            )
            label_keys.forEach(
                k => {
            ...
            promises.push(
                LabelService.update(label_payload.id, label_payload).then(
                    (r) => {
                      return r.data.id
                    }
                )
    ...
    createLabels(labels, task_type) {
      ...
      labels.forEach(
          l => {
            ...
            promises.push(
                LabelService.create(label_payload).then(
                    (r) => {
                      return r.data.id
                    }
                )
    ...
    edit() {
      ...
      if (Object.keys(this.schema.properties).includes('labels')) {
        let labels_to_create = t.labels.filter(
            l => {
              return l.id == null
            }
        )

        let labels_to_keep = t.labels.filter(
            l => l.id != null
        )

        create_labels_promises = this.createLabels(labels_to_create, t.resourcetype)
        edit_labels_promises = this.editLabels(labels_to_keep, t.resourcetype)
      }

      Promise.all(
          create_labels_promises.concat(edit_labels_promises)
      ).then(
          (label_ids) => {
            /*
            let task_payload = {
