<template>
  <div class="task-form-container">
    ...
    />
    <br>
    <div id="entity-info" v-if="!readOnly">
    ...
    <div>
      <div v-for="(val, key) in grouped_selected_labels" :key="key" style="margin-bottom:5px;">
  ...
  created() {
    ...
    Promise.all(promises).then(
        () => {
          ...
          if (!this.readOnly) {
            this.$store.commit('entities/ADD_ANNOTATED_ENTITIES', this.selected_labels)
          }
        }
    )
  },
  ...
  watch: {
    showAnnotatedEntities() {
      if (this.showAnnotatedEntities) {
        this.$store.commit('entities/SET_ANNOTATED_ENTITIES', this.selected_labels)
      } else {
    ...
    annotated_entities() {
      ...
      this.selected_labels = this.annotated_entities.filter(
          (e) => {
            ...
            (selected_label) => {
              ...
              if (is_label_in_labels_to_add) {
                ...
                if (!selected_label.id) { //&& !this.labels_to_add.find(l => l.name == selected_label.name && l.start_offset == selected_label.start_offset)
                  ...
                  let original_label = this.entity_tasks.find(t => t.id == selected_label.task)
